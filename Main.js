...
const calcSlope = (elasticity) => {
  if (elasticity >= 99) return 0.001; // Vollkommen elastisch (waagerecht)
  if (elasticity <= 1) return 1000;   // Vollkommen unelastisch (senkrecht)
  return Math.tan(((100 - elasticity) / 100) * (Math.PI / 2));
};

const calcIntercept = (type, position, b) => {
  // Der "Position"-Regler (0-100) verschiebt einen diagonalen Ankerpunkt.
  // Das garantiert, dass unelastische (senkrechte) Kurven nach links/rechts wandern
  // und elastische (waagerechte) nach oben/unten.
  if (type === 'demand') {
    // Ankerpunkt: Q = position, P = position
    // a = P + bQ => a = position + b * position
    return position + (b * position);
  } else {
    // Ankerpunkt: Q = position, P = 100 - position (mehr Angebot = tieferer Preis)
    // a = P - bQ => a = (100 - position) - b * position
    return (100 - position) - (b * position);
  }
};

export default function MultiVWLEditor() {
  // --- Dashboard State ---
...
// --- SUB-COMPONENT: Single Chart Renderer ---
        ...
        <svg ref={svgRef} viewBox={`0 0 ${width} ${height}`} className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
          ...
          <g clipPath={`url(#clip-${chart.id})`}>
            ...
            {mathCurves.map((curve) => {
              // Berechne Punkte anhand der P-Grenzen, das verhindert SVG-Fehler bei steilen Kurven
